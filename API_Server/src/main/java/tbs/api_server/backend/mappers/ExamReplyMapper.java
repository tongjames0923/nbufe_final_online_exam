...
public interface ExamReplyMapper
{
    ...
    int insertReply(int exam_id, @NonNull String examnumber,@NonNull String person_id, byte[] reply_file);

    @Select("SELECT * FROM `exam_reply` WHERE `exam_id`=#{param1} LIMIT #{from},#{num}")
    List<ExamReply> getReplys(int examid,int from,int num);
