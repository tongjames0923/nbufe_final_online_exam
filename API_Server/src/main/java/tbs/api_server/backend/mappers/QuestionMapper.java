...
public interface QuestionMapper
{
    ...
    @Select("SELECT `que_id`,`que_type`, `que_creator`, `que_alter_time`, `publicable`, `tag`, `use_time`, `answerd`, `answerd_right`,`title` FROM `question` LIMIT #{from},#{num}")
    List<Question> getQuestions(int from, int num);

    @Select("SELECT `que_id`,`que_type`, `que_creator`, `que_alter_time`, `publicable`, `tag`, `use_time`, `answerd`, `answerd_right`,`title` FROM `question` WHERE `tag`=#{tag} LIMIT #{from},#{num}")
    List<Question> getQuestionsByTag(int tag, int from, int num);

    @Select("SELECT `que_id`,`que_type`, `que_creator`, `que_alter_time`, `publicable`, `tag`, `use_time`, `answerd`, `answerd_right`,`title` FROM `question` WHERE `que_type`=#{type} LIMIT #{from},#{num}")
    List<Question> getQuestionsByType(int type, int from, int num);

    @Select("SELECT `que_id`,`que_type`, `que_creator`, `que_alter_time`, `publicable`, `tag`, `use_time`, `answerd`, `answerd_right`,`title` FROM `question` WHERE `title` LIKE %${title}% LIMIT #{from},#{num}")
    List<Question> findQuestionByTitle(String title,int from,int num);

    @Select("SELECT `que_file` FROM `question` WHERE `que_id`=#{quesid}")
    ...
    @Select("SELECT `que_id`,`que_type`, `que_creator`, `que_alter_time`, `publicable`, `tag`, `use_time`, `answerd`, `answerd_right`,`title` FROM `question` WHERE `que_id`=#{que_id} and `que_creator`=#{user}")
