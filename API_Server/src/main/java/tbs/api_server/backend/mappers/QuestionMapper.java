...
public interface QuestionMapper
{
    ...
    @Insert("INSERT INTO `question` (`que_type`, `que_creator`, `que_alter_time`, `que_file`, `publicable`,  `use_time`, `answerd`, `answerd_right`,`title`) VALUES (#{que_type},#{creator_id}, CURRENT_TIMESTAMP, #{que_file}, #{isopen}, #{tagid}, '0', '0', '0',#{title})")
    int insertQuestion(int que_type, int creator_id, byte[] que_file,String title, Integer isopen);

    @Select("SELECT `que_id`,`que_type`, `que_creator`, `que_alter_time`, `publicable`,`use_time`, `answerd`, `answerd_right`,`title` FROM `question` LIMIT #{from},#{num} FOR UPDATE")
    List<Question> getQuestions(int from, int num);

    @Select("SELECT `que_id`,`que_type`, `que_creator`, `que_alter_time`, `publicable`, `use_time`, `answerd`, `answerd_right`,`title` FROM `question` WHERE `que_type`=#{type} LIMIT #{from},#{num} FOR UPDATE")
    List<Question> getQuestionsByType(int type, int from, int num);

    @Select("SELECT `que_id`,`que_type`, `que_creator`, `que_alter_time`, `publicable`,`use_time`, `answerd`, `answerd_right`,`title` " +
