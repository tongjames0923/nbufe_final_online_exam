...
public class QuestionImp implements QuestionService
{
    ...
    @Override
    public ServiceResult uploadQuestion(int que_type,String title, int creator_id, byte[] que_file, Integer isopen) throws BackendError {
        int c= mp.insertQuestion(que_type, creator_id, que_file,title, isopen);
        if(c>0)
        {
